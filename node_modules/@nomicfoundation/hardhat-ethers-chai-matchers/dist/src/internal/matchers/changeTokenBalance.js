...
function validateInput(obj, token, accounts, balanceChanges) {
    try {
        ...
        if (Array.isArray(balanceChanges) &&
            ...
            chaiAssert.fail(`The number of accounts (${accounts.length}) is different than the number of expected balance changes (${balanceChanges.length})`);
        }
    }
    catch (e) {
...
function checkToken(token, method) {
    if (!isObject(token) || token === null || !("interface" in token)) {
        chaiAssert.fail(`The first argument of "${method}" must be the contract instance of the token`);
    }
...
export async function getBalanceChange(ethers, transaction, token, account) {
    ...
    assertIsNotNull(txReceipt, "Transaction's receipt cannot be fetched from the network");
    const txBlockNumber = txReceipt.blockNumber;
    const block = await ethers.provider.getBlock(txReceipt.blockHash, false);
    assertIsNotNull(block, "The transaction's block cannot be fetched from the network");
    chaiAssert.equal(block.transactions.length, 1, "There should be only 1 transaction in the block");
