...
export function decodeReturnData(returnData) {
    ...
    if (returnData === "0x") {
        ...
        try {
            ...
            ensureError(cause);
            try {
                chaiAssert.fail(`There was an error decoding "${encodedReason}" as "string". Reason: ${cause.message}`);
            }
            catch (e) {
                ensureError(e);
                e.cause = cause;
        ...
        try {
            ...
            ensureError(cause);
            try {
                chaiAssert.fail(`There was an error decoding "${encodedReason}" as a "uint256". Reason: ${cause.message}`);
            }
            catch (e) {
                ensureError(e);
                e.cause = cause;
