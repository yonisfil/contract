...
import { revertedWithCustomErrorWithArgs, REVERTED_WITH_CUSTOM_ERROR_CALLED, } from "./reverted/revertedWithCustomError.js";
/**
 ...
 */
export function anyUint(i) {
    if (typeof i === "number") {
        if (i < 0) {
            // eslint-disable-next-line no-restricted-syntax -- keep the original chai error structure
            throw new AssertionError(`anyUint expected its argument to be an unsigned integer, but it was negative, with value ${i}`);
        }
        return true;
    }
    else if (isBigInt(i)) {
        ...
        if (bigInt < 0) {
            // eslint-disable-next-line no-restricted-syntax -- keep the original chai error structure
            throw new AssertionError(`anyUint expected its argument to be an unsigned integer, but it was negative, with value ${bigInt}`);
        }
        return true;
    }
    // eslint-disable-next-line no-restricted-syntax -- keep the original chai error structure
    throw new AssertionError(`anyUint expected its argument to be an integer, but its type was '${typeof i}'`);
}
export function supportWithArgs(Assertion, chaiUtils) {
    Assertion.addMethod("withArgs", function (...expectedArgs) {
        ...
        const onSuccess = (resolvedExpectedArgs) => {
            if (emitCalled) {
                return emitWithArgs(this, Assertion, chaiUtils, resolvedExpectedArgs, onSuccess);
            }
            else {
                return revertedWithCustomErrorWithArgs(this, Assertion, chaiUtils, resolvedExpectedArgs, onSuccess);
            }
        };
...
function validateInput(chaiUtils) {
    try {
        ...
        const revertedWithCustomErrorCalled = chaiUtils.flag(this, REVERTED_WITH_CUSTOM_ERROR_CALLED) === true;
        if (!emitCalled && !revertedWithCustomErrorCalled) {
            chaiAssert.fail("withArgs can only be used in combination with a previous .emit or .revertedWithCustomError assertion");
        }
        if (emitCalled && revertedWithCustomErrorCalled) {
            chaiAssert.fail("withArgs called with both .emit and .revertedWithCustomError, but these assertions cannot be combined");
        }
        return { emitCalled };
    }
    catch (e) {
