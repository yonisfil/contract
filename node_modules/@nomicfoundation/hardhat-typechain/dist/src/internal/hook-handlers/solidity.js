    ...
    const handlers = {
        async build(context, rootFilePaths, options, next) {
            ...
            if (context.config.solidity.splitTestsCompilation) {
                artifactPaths = Array.from(await context.artifacts.getAllArtifactPaths());
            }
            else {
...
async function getContractArtifactPaths(context) {
    ...
    return Promise.all(contractFqns.map((fqn) => context.artifacts.getArtifactPath(fqn)));
}
