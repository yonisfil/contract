...
export async function generateTypes(rootPath, config, noTypechain, artifactsPaths) {
 ...
 */
function removePrettierTransformerIfPresent(outputTransformers) {
    ...
    const prettierIndex = outputTransformers.findIndex((item) => item.name === PRETTIER_TRANSFORMER_NAME);
    if (prettierIndex !== -1) {
        const removedTransformer = outputTransformers.splice(prettierIndex, 1)[0];
        assertHardhatInvariant(removedTransformer.name === PRETTIER_TRANSFORMER_NAME, "TypeChain output transformer arrays changed in an unexpected way");
    }
}
/**
 ...
 */
function addCompiledFilesTransformerIfAbsent(outputTransformers) {
    if (
    ...
    const compiledFilesTransformer = (output, _services, _config) => {
        ...
        modifiedContent = modifiedContent.replaceAll('from "@nomicfoundation/hardhat-ethers/types"', 'from "@nomicfoundation/hardhat-ethers/types"');
        // Fixes the module augmentation to use the types declared in "ethers-v3"
        modifiedContent = modifiedContent.replaceAll('declare module "hardhat/types/runtime"', 'declare module "@nomicfoundation/hardhat-ethers/types"');
 ...
 */
export function addJsExtensionsIfNeeded(content) {
    const jsExtensionRegex = /^import\s+(.*?)\s+from\s+(['"])(\.[^'"]*)(?<!\.js)\2;?$/gm;
    return content.replace(jsExtensionRegex, (_match, imports, quote, importPath) => `import ${imports} from ${quote}${importPath}/index.js${quote};`);
}
...
//   ...
// }
function addSupportForAttachMethod(modifiedContent) {
  ...
  `;
    ...
    modifiedContent = modifiedContent.replace("/* eslint-disable */", '/* eslint-disable */\nimport type { Addressable } from "ethers";');
    return modifiedContent;
}
/**
