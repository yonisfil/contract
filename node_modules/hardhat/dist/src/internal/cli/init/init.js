...
import { copy, ensureDir, exists, getAllFilesMatching, isDirectory, mkdir, readJsonFile, writeJsonFile, } from "@nomicfoundation/hardhat-utils/fs";
...
import { getDevDependenciesInstallationCommand, getPackageManager, installsPeerDependenciesByDefault, } from "./package-manager.js";
import { promptForMigrateToEsm, promptForForce, promptForInstall, promptForTemplate, promptForUpdate, promptForWorkspace, promptForHardhatVersion, } from "./prompt.js";
...
export async function initHardhat(options) {
    try {
        ...
        const hardhatVersion = options?.hardhatVersion ?? (await promptForHardhatVersion());
        ...
        const [template, projectTypeAnalyticsPromise] = await getTemplate(hardhatVersion, options?.template);
...
export async function printWelcomeMessage() {
    ...
    console.log(chalk.cyan(`👷 Welcome to ${HARDHAT_NAME} v${hardhatVersion} 👷\n`));
    ...
    try {
        ...
        if (hardhatVersion !== latestHardhatVersion) {
            console.warn(chalk.yellow.bold(`⚠️ You are using an outdated version of Hardhat. The latest version is v${latestHardhatVersion}. Please consider upgrading to the latest version before continuing with the project initialization. ⚠️\n`));
        }
    }
    catch (error) {
        ensureError(error);
        ...
        console.warn(chalk.yellow.bold(`⚠️ We couldn't check if you are using the latest version of Hardhat. Please consider upgrading to the latest version if you are not using it yet. ⚠️\n`));
    }
}
/**
 ...
 */
export async function getTemplate(hardhatVersion, template) {
    ...
    const projectTypeAnalyticsPromise = sendProjectTypeAnalytics(hardhatVersion, template);
...
export async function validatePackageJson(workspace, templatePkg, migrateToEsm) {
    ...
    if (templateHardhatVersion.startsWith("^2")) {
        await spawn([packageManagerToUse, "pkg", "delete", "dependencies.hardhat"].join(" "), [], {
            cwd: workspace,
            shell: true,
            stdio: "inherit",
        });
...
export async function copyProjectFiles(workspace, template, force) {
    ...
    const matchingRelativeWorkspacePaths = await getAllFilesMatching(workspace, (file) => {
        const relativeWorkspacePath = path.relative(workspace, file);
        const relativeTemplatePath = relativeWorkspaceToTemplatePath(relativeWorkspacePath);
 ...
 */
export async function installProjectDependencies(workspace, template, install, update) {
    ...
    if (Object.keys(dependenciesToInstall).length !== 0) {
        ...
        const command = getDevDependenciesInstallationCommand(packageManager, dependenciesToInstall);
    ...
    if (dependenciesToUpdate.length !== 0) {
        // Retrieve the package manager specific installation command
        const command = getDevDependenciesInstallationCommand(packageManager, dependenciesToUpdate);
        const commandString = command.join(" ");
...
function showStarOnGitHubMessage() {
    console.log(chalk.cyan("Give Hardhat a star on Github if you're enjoying it! ⭐️✨"));
    console.log();
