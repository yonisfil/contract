...
import { FileNotFoundError, readdir, readJsonFile, remove, writeJsonFile, } from "@nomicfoundation/hardhat-utils/fs";
...
export function logSnapshotCheatcodesSection(result, logger = console.log) {
    ...
    if (isFirstTimeWrite) {
        logger();
        logger(chalk.green("  No existing snapshots found. Snapshot cheatcodes written successfully"));
        logger();
        return;
    }
    if (hasChanges) {
...
export function printSnapshotCheatcodeChanges(changes, logger = console.log) {
    for (let i = 0; i < changes.length; i++) {
        ...
        logger(chalk.grey(`    Actual:   ${change.actual} (`) +
            formattedGasChange +
            chalk.grey(")"));
        if (!isLast) {
            logger();
