...
(function (CompilerPlatform) {
    ...
    CompilerPlatform["LINUX_ARM64"] = "linux-arm64";
...
export class CompilerDownloaderImplementation {
    ...
    constructor(platform, compilersDir, downloadFunction = download) {
        ...
        this.#mutexCompilerList = new MultiProcessMutex(path.join(compilersDir, "compiler-download-list"));
    ...
    async downloadCompiler(version) {
        ...
        const mutex = new MultiProcessMutex(path.join(this.#compilersDir, `compiler-download-${version}`));
            ...
            for (let i = 0; i <= DEFAULT_COMPILER_DOWNLOAD_RETRY_COUNT; i++) {
                try {
                    ...
                    if (i === DEFAULT_COMPILER_DOWNLOAD_RETRY_COUNT) {
                        ...
                        const attempt = i + 1;
                        log(`Download or verification failed for solc ${version}, retrying (attempt ${attempt} of ${DEFAULT_COMPILER_DOWNLOAD_RETRY_COUNT})`);
                        await new Promise((resolve) => setTimeout(resolve, DEFAULT_COMPILER_DOWNLOAD_RETRY_DELAY_MS));
                    }
                }
            }
    ...
    async #downloadCompilerList() {
        ...
        await this.#downloadFunction(`${COMPILER_REPOSITORY_URL}/${this.#platform}/list.json`, downloadPath);
        ...
        if (this.#platform === CompilerPlatform.LINUX_ARM64) {
            ...
            const officialCompilerList = await readJsonFile(downloadPath);
            await this.#downloadFunction(`${LINUX_ARM64_REPOSITORY_URL}/list.json`, downloadPath);
            // add missing information and an explicit URL for download
            const armLinuxcompilerList = await readJsonFile(downloadPath);
            ...
            officialCompilerList.builds = officialCompilerList.builds.concat(armLinuxcompilerList.builds);
    ...
    async #downloadAndVerifyCompiler(build) {
        let downloadPath = "";
        try {
