...
const buildAction = async (args, hre) => {
    const buildProfile = hre.globalOptions.buildProfile ?? args.defaultBuildProfile;
    ...
    if (args.noContracts && partitionedFiles.contractRootPaths.length > 0) {
        throw new HardhatError(HardhatError.ERRORS.CORE.SOLIDITY.INCOMPATIBLE_FILES_WITH_BUILD_FLAGS, {
    ...
    if (hre.config.solidity.splitTestsCompilation) {
        ...
        const testRootPaths = [];
        const shouldBuildContracts = !args.noContracts &&
        ...
        if (shouldBuildContracts) {
            ...
            assertHardhatInvariant(contractBuildResults.testRootPaths.length === 0, "The contracts scope should build no test in split test compilation mode");
            contractRootPaths.push(...contractBuildResults.contractRootPaths);
            testRootPaths.push(...contractBuildResults.testRootPaths);
        }
        const shouldBuildTests = !args.noTests &&
        ...
        if (shouldBuildTests) {
            ...
            assertHardhatInvariant(testBuildResults.contractRootPaths.length === 0, "The tests scope should build no contract in split test compilation mode");
 ...
 */
async function runSolidityBuild({ buildProfile, files, force, isUnifiedModeOrScope, noContracts, noTests, quiet, solidity, }) {
    const scope = isUnifiedModeOrScope === true ? "contracts" : isUnifiedModeOrScope;
    const { isFullBuild, contractRootPaths, testRootPaths } = await getRootsToBuild({
        solidity,
    ...
    const results = await solidity.build([...contractRootPaths, ...testRootPaths], {
        force,
        buildProfile,
        quiet,
        scope,
    ...
    if (builtRootPaths.length === preBuildRoots.size &&
 ...
 * compilation. i.e. it should only include files of the scope being used.
 */
async function getRootsToBuild({ solidity, isUnifiedModeOrScope, files, noTests, noContracts, }) {
 ...
 */
async function getRootsToBuildInUnifiedMode({ files, noContracts, noTests, solidity, }) {
    ...
    const partitionedRootPaths = await partitionRootPathsByScope(solidity, rootFilePaths);
    return {
        isFullBuild,
 ...
 * being used.
 */
async function getRootsToBuildForScope({ files, scope, solidity, }) {
 ...
 */
async function partitionRootPathsByScope(solidity, rootPaths) {
    const contractRootPaths = [];
    const testRootPaths = [];
 ...
 */
function normalizeRootPaths(files) {
    return files.map((f) => {
        if (isNpmRootPath(f)) {
            return f;
        }
