    ...
    static async create(inputUserConfig, userProvidedGlobalOptions, projectRoot, unsafeOptions) {
        ...
        if (!configResolutionResult.success) {
            if (configResolutionResult.configValidationErrors !== undefined) {
                throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.INVALID_RESOLVED_CONFIG, {
                    errors: `\t${configResolutionResult.configValidationErrors
                        .map((error) => `* Resolved config error in config.${error.path.join(".")}: ${error.message}`)
                        .join("\n\t")}`,
                });
            }
...
export async function resolveUserConfigToHardhatConfig(inputUserConfig, hooks, resolvedProjectRoot, userProvidedConfigPath, resolvedPlugins) {
    ...
    const resolvedConfigValidationResults = await hooks.runSequentialHandlers("config", "validateResolvedConfig", [config]);
